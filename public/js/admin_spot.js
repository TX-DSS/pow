...
$(function(){
    ...
    function deleteSpot() {
        var id = editor.getSpotInfo()._id;
        if ( !id ) return;

        $.ajax({
            method: "POST",
            url: "http://api.pow.com:3000/spot/delete",
            data: {_id: id}
        }).done(function(msg) {
            console.log(msg);
    ...
    function querySpot() {
        ...
        $.ajax({
            ...
            if ( rest && rest.length == 1 ) {
                $('#createSpotBtn').attr('disabled', 'disabled');
                $('#modifySpotBtn').removeAttr('disabled');
                $('#deleteSpotBtn').removeAttr('disabled');
                editor.setSpotInfo(rest[0]);
            } else {
                alert('No Data');
                $('#createSpotBtn').attr('disabled', 'disabled');
                $('#modifySpotBtn').attr('disabled', 'disabled');
                $('#deleteSpotBtn').attr('disabled', 'disabled');
            }
        });
    ...
    function newSpot() {
        $('#queryNo').val('');
        editor.initSpotInfo();
        $('#createSpotBtn').removeAttr('disabled');
        $('#modifySpotBtn').attr('disabled', 'disabled');
        $('#deleteSpotBtn').attr('disabled', 'disabled');
    }

    $('#querySpotBtn').bind('click', querySpot);
    $('#newSpotBtn').bind('click', newSpot);
